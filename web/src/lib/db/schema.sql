-- Convo D1 schema — source of truth (Month 1 + v3 extras)
-- Run: npx wrangler d1 execute convo-production --local --file=web/src/lib/db/schema.sql
...
-- 9. playbook_activations (which automations are active per org)
...
-- 10. fas_events (feature usage for FAS score)
...
-- 11. payment_links (Razorpay UPI, amount in paise)
...
-- 13. conversation_tags (PRIMARY KEY for no duplicate tags)
...
-- 14. consent_records (DPDP opt-in/opt-out)
...
-- 15. audit_log (immutable DPDP audit trail)
